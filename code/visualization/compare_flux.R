...
all_results <- list()
for (MAP in map){
  ##### plot each replication: true and estimated values for each channel #####
  ...
  all_results[[MAP]] <- list(
    data = x,
    true_sinai = true_sinai,
    true_mandeb = true_mandeb,
    true_gibraltar = true_gibraltar
  )
  ...
  for (i in 1:n) {
    ...
    f_data <- read.csv(
      sprintf(
        "data/flux/flux_%s_%s_%s_%s_%s.csv",
        WORLD, source_pop, end_gen, MAP, rep_id
      ),
      row.names = 1
    )

    true_mat <- matrix(f_data$true_flux, 177, 177)
  ...
  flux_features <- data.frame(
    sinai = true_sinai,
    mandeb = true_mandeb,
    gibraltar = true_gibraltar
  )
...
##### plot each replication: one channel at a time #####
channels <- c("sinai","mandeb","gibraltar")
for(channel in channels){
  friction <- all_results[["friction"]]$data
  naive <- all_results[["naive"]]$data
  true_flux <- all_results[["friction"]][[paste0("true_",channel)]]
  n <- length(true_flux)
  output_file <- sprintf("output/figures/compare_flux_%s_%s_%s_%s.png",WORLD,source_pop,end_gen,channel)
  
  png(output_file,width = 10,height = 5,units = "in",res = 300,bg = "transparent")
  plot(0,0,type="n",xlim=c(1,n),ylim=c(0,1),xaxt="n",las=1,xlab="",ylab="", bty="n")
  axis(1,at=1:n,labels=1:n,las=2,cex.axis=1.4)
  mtext("flux", side=2, line=2.5, cex=1.5)
  mtext("replications", side=1, line=3.5, cex=1.5)
  
  # true value
  lines(1:n,true_flux,col="black",lwd=2,lty=1)
